...
function save_rol_and_update(e) {
  ...
  $.ajax({
    url: "../../controllers/SeccionController.php?enpoint=save_and_update_secciones",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
    success: function (data) {
      $("#tabla_secciones").DataTable().ajax.reload();
...
$(document).ready(function () { 
  ...
  $("#tabla_secciones").DataTable({
    ...
    ajax: {
      url: "../../controllers/SeccionController.php?enpoint=list_secciones",
      type: "post",
      data: { true: true },
    },
    bDestroy: true,
...
function editar(Id) { 
  $.post("../../controllers/SeccionController.php?enpoint=list_seccion_id", {id_seccion: Id}, function(data){
      console.log(data); 
   ...
   $('#lbltitulo').html('Actualizar Seccion');
...
swal.fire({
  ...
  if (result.value) {
    ...
    $.post("../../controllers/SeccionController.php?enpoint=delete", {id_seccion: Id}, function(data){
...
$(document).on("click","#btn_nuevo_registro",function(){
    ...
    $('#descripcion').val('');
     $('#estado').val(''); 
    $('#lbltitulo').html('Registrar Nueva Seccion');
